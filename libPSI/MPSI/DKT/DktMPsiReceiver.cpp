  ...
  span<Channel> chls)
{
  ...
  auto routine = [&](u64 t) {
    ...
    REccPoint gg;
    gg.randomize(ZeroBlock);
    REccPoint ggg;
    gg.randomize(OneBlock);
    auto g2 = gg + ggg;
    ...
    for (u64 i = myInputStartIdx; i < myInputEndIdx;) {
      ...
      for (u64 j = 0; j < curStepSize; ++j, ++i) {
        ...
        Ms.back().toBytes(iter);
        iter += Ms.back().sizeBytes();
        Ns.back().toBytes(iter);
    ...
    for (u64 i = myInputStartIdx, ii = 0; i < myInputEndIdx;) {
      ...
      for (u64 j = 0; j < curStepSize; ++j, ++i, ++ii) {
        ...
        sigmaPhis[ii].toBytes(iter);
        iter += sigmaPhis[ii].sizeBytes();
    ...
    for (u64 i = myInputStartIdx; i < myInputEndIdx;) {
      ...
      for (u64 j = 0; j < curStepSize; ++j, ++i) {
        ...
        sigma2As.back().fromBytes(iter);
        iter += sigma2As.back().sizeBytes();
      }
    }
    ...
    if (t == 0)
      hashs.reserve(inputs.size());
    else
      hashVec.resize(subsetInputSize);
