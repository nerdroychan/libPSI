...
void Rr17aMPsiSender::init(u64 n, u64 statSec, Channel &chl0, NcoOtExtSender &ots, NcoOtExtReceiver &otRecv, block seed, double binScaler, u64 inputBitSize)
{
  std::vector<Channel> c{ chl0 };
  init(n, statSec, c, ots, otRecv, seed, binScaler, inputBitSize);
}

void Rr17aMPsiSender::init(u64 n, u64 statSecParam, span<Channel> chls, NcoOtExtSender &otSend, NcoOtExtReceiver &otRecv, block seed, double binScaler, u64 inputBitSize)
{
  ...
  if (otSend.hasBaseOts() == false || otRecv.hasBaseOts() == false) {
    ...
    BitVector recvChoice(baseOtCount + gOtExtBaseOtCount);
