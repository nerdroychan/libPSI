...
void BalancedIndex::init(u64 inputSize, u64 maxBinSize, u64 numDummies, __attribute__((unused)) u64 statSecParam)
{
  numIters = 0;
...
void BalancedIndex::check()
{
  for (u64 idxBin = 0; idxBin < mNumBins; ++idxBin) {
    if (mBins[idxBin].cnt > mMaxBinSize)
      std::cout << idxBin << "\t cnt:" << mBins[idxBin].cnt << "\t"
                << "mMaxBinSize " << mMaxBinSize << "\n";
  }
}
