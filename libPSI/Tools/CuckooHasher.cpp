  ...
  for (u64 i = 0; i < inputIdxs.size(); ++i) {
    for (u64 j = 0; j < mParams.mNumHashes; ++j) {
      ...
      if ((mHashesView.data() + inputIdxs[i] * width)[j] != u64(-1)) {
        std::cout << IoStream::lock << "cuckoo index " << inputIdxs[i] << " already inserted" << std::endl
                  << IoStream::unlock;
        throw std::runtime_error(LOCATION);
      }
#endif// ! NDEBUG
...
u64 CuckooHasher::find(span<u64> hashes)
{
  if (mParams.mNumHashes == 2) {
    ...
    if (val[0] != u64(-1)) {
      ...
      bool match =
        (mHashesView[itemIdx][0] == hashes[0]) && (mHashesView[itemIdx][1] == hashes[1]);
  ...
  if (mParams.mNumHashes == 2) {
    ...
    for (u64 i = 0; i < hashes.bounds()[0]; ++i) {
      if (w.findVal[i][0] != u64(-1)) {
        ...
        bool match =
          (mHashesView[itemIdx][0] == hashes[i][0]) && (mHashesView[itemIdx][1] == hashes[i][1]);
    ...
    while (i < mStash.size() && mStash[i].isEmpty() == false) {
      ...
      if (val != u64(-1)) {
        ...
        for (u64 j = 0; j < hashes.bounds()[0]; ++j) {
          ...
          bool match =
            (mHashesView[itemIdx][0] == hashes[j][0]) && (mHashesView[itemIdx][1] == hashes[j][1]);
