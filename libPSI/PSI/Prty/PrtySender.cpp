    ...
    for (u64 i = binStartIdx; i < binEndIdx; i += stepSize) {
      ...
      for (u64 k = 0; k < curStepSize; ++k) {
        ...
        for (int idx = 0; idx < realNumRows; idx++) {
          ...
          memcpy(globalHash[hashIdx].data() + permute[hashIdx][idxPermuteDone[hashIdx]++] * hashMaskBytes, (u8 *)&cipher[0], hashMaskBytes);
        }
      ...
      for (u64 k = 0; k < curStepSize; ++k) {
        ...
        for (int idx = 0; idx < realNumRows; idx++) {
          ...
          if (isMultiThreaded) {
            ...
            memcpy(globalHash[hashIdx].data() + permute[hashIdx][idxPermuteDone[hashIdx]++], (u8 *)&cipher[0], sizeof(block));
          } else
            memcpy(globalHash[hashIdx].data() + permute[hashIdx][idxPermuteDone[hashIdx]++], (u8 *)&cipher[0], sizeof(block));
        }
