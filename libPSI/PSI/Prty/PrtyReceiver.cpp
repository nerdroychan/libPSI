      ...
      for (u64 k = 0; k < curStepSize; ++k) {
        ...
        for (u64 idx = 0; idx < mBalance.mBins[bIdx].cnt; ++idx) {
          ...
          if (isMultiThreaded) {
            ...
            localMasks[mBalance.mBins[bIdx].hashIdxs[idx]].emplace(*(u64 *)&cipher[0], std::pair<block, u64>(cipher[0], mBalance.mBins[bIdx].idxs[idx]));
          } else {
            localMasks[mBalance.mBins[bIdx].hashIdxs[idx]].emplace(*(u64 *)&cipher[0], std::pair<block, u64>(cipher[0], mBalance.mBins[bIdx].idxs[idx]));
          }
    ...
    for (u64 i = binStartIdx; i < binEndIdx; i += stepSize) {
      ...
      for (u64 k = 0; k < curStepSize; ++k) {
        ...
        for (u64 idx = 0; idx < mBalance.mBins[bIdx].cnt; ++idx) {
          ...
          if (isMultiThreaded) {
            std::lock_guard<std::mutex> lock(mtx);
            localMasks[mBalance.mBins[bIdx].hashIdxs[idx]].emplace(*(u32 *)&cipher[0], std::pair<block, u64>(cipher[0], mBalance.mBins[bIdx].idxs[idx]));
          } else {
            localMasks[mBalance.mBins[bIdx].hashIdxs[idx]].emplace(*(u32 *)&cipher[0], std::pair<block, u64>(cipher[0], mBalance.mBins[bIdx].idxs[idx]));
          }
        }
