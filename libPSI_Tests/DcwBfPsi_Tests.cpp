...
void DcwRBfPsi_FullSet_Test_Impl()
{
  ...
  if (recvPSIs.mIntersection.size() != setSize) {
    ...
    throw UnitTestFail("bad size, expected:" + std::to_string(setSize) + ", actual:" + std::to_string(recvPSIs.mIntersection.size()));
  }
}
...
void DcwRBfPsi_SingltonSet_Test_Impl()
{
  ...
  auto recvThrd = std::thread([&]() {
    for (u64 j = 0; j < repeatCount; ++j) {
      ...
      if (recv.mIntersection.size() != 1 || recv.mIntersection[0] != 0)
        throw UnitTestFail();
    }
  });
  sendThrd.join();
...
void DcwRBfPsi_EmptySet_Test_Impl()
{
  throw oc::UnitTestSkipped("ENABLE_DCW_PSI not defined");
}
