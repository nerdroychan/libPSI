...
void Psi_drrn_FullSet_Test_Impl()
{
  ...
  if (client.mIntersection.size() != clientSetSize) {
    ...
    throw UnitTestFail("Known issue with incorrect result (mismatched set size). Ok to ignore for benchmarking...");
    return;
  }
  ...
  for (u64 i = 0; i < clientSetSize; ++i) {
    ...
    if (b) {
      ...
      throw UnitTestFail("Known issue with incorrect result (item not found). Ok to ignore for benchmarking...");
      return;
    }
  }
